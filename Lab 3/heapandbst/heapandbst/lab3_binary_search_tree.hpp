...
protected:
	...
	// PURPOSE: Deletes nodes recursively
	void delete_recursive(TaskItem* node);

	// PURPOSE: Overloaded tree height function to use recursion
	unsigned int height(TaskItem* node) const;

	// PURPOSE: Overloaded exists helper function to use recursion
	bool exists(BinarySearchTree::TaskItem val, BinarySearchTree::TaskItem* node) const;

	// PURPOSE: Overloaded insert helper function to use recursion
	TaskItem* insert(BinarySearchTree::TaskItem val, BinarySearchTree::TaskItem* node);

	// PURPOSE: Finds the minimum value provided a node
	TaskItem* find_min(TaskItem* node);

	// PURPOSE: Remove helper function to use recursion
	TaskItem* remove(BinarySearchTree::TaskItem val, BinarySearchTree::TaskItem* node);
